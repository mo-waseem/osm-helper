...
def haversine_distance(location_a, location_b): # (source_lat, source_lng), (destination_lat, destination_lng)
    def hav(angle):
        return sin(angle / 2) ** 2
    # we convert from decimal degree to radians
    lat_a, lon_a, lat_b, lon_b = radians(location_a[0]), radians(location_a[1]), radians(location_b[0]), \
